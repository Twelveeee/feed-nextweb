...
import { AddFeedSourceRequest } from '@/types';

export async function POST(request: NextRequest) {
  try {
    const body: AddFeedSourceRequest = await request.json();
    ...
    // 请求真实后端
    const response = await fetch(`${apiBaseUrl}/add_feed_source`, {
      method: 'POST',
      headers,
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: '添加订阅源失败', details: errorText },
